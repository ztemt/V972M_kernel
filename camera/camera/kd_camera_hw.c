...
u32 pinSet[3][8] = {
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV9740_MIPI_YUV,currSensorName)))
	{
		PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_OV9740_MIPI_YUV \n");
		PK_DBG("[ON_general 2.8V]sensorIdx:%d \n",SensorIdx);
		...
		if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
		{
			PK_DBG("[CAMERA SENSOR] PDN STBY pin setting\n");
			if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! \n");}
			if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! \n");}
			/*if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_OFF])){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}*/
			if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_ON])){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
	...
	else if(currSensorName && (0 == strcmp(SENSOR_DRVNAME_HI542MIPI_RAW,currSensorName)))
		{
				PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_HI542MIPI_RAW \n");
				PK_DBG("[ON_general 2.8V]sensorIdx:%d \n",SensorIdx);
			if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name))
       		 {
            	PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
    ...
    else
    {
		...
		{
			PK_DBG("kdCISModulePower--off get in---SENSOR_DRVNAME_OV9740_MIPI_YUV \n");
			printk("kdCISModulePower--off get in---SENSOR_DRVNAME_OV9740_MIPI_YUV \n");
			...
			if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_A2,mode_name))
			{
				...
				{
					PK_DBG("kdCISModulePower--off get in---SENSOR_DRVNAME_HI542MIPI_RAW \n");
